...
上市公司财报 PDF 自动搜索与下载工具，支持 [雪球](https://stockn.xueqiu.com)（主源）、[同花顺](https://notice.10jqka.com.cn)（备选源）以及 [股转系统](https://www.neeq.com.cn)（新三板），可通过 [Claude Code](https://claude.com/claude-code) 斜杠命令或 Claude 插件两种方式使用。
...
# 新三板（NEEQ）：脚本自动搜索股转系统官网，无需提供 URL
/download-report 833442 2018 年报        # 江苏铁科 2018 年报
```
...
| 新三板 (NEEQ) | 4/8/92 开头的 6 位数字 | `833442`（江苏铁科） |

## 支持的报告类型

| 报告类型 | 用户输入 | 适用市场 | 典型发布时间 |
|---------|---------|---------|------------|
| 年报 | `年报` / `annual` | A 股、港股、新三板 | 次年 3-4 月 |
| 中报 | `中报` / `interim` | A 股、港股、新三板 | 当年 8-9 月 |
| 一季报 | `一季报` / `Q1` | 仅 A 股 | 当年 4 月 |
| 三季报 | `三季报` / `Q3` | 仅 A 股 | 当年 10 月 |

> 新三板仅支持年报、中报（一季报/三季报会给出明确提示）。
...
2. **搜索报告**：A 股/港股通过 WebSearch 在 `stockn.xueqiu.com` 搜索，无结果时回退到 `notice.10jqka.com.cn`（同花顺）；新三板由脚本自动搜索 `neeq.com.cn` 公告 API（自动处理反爬 cookie）
3. **筛选结果**：排除摘要、审计报告等非正文文件，选择最匹配的 PDF
4. **下载文件**：通过 Python 脚本流式下载，支持重试和 PDF 格式校验

## 注意事项

- A 股/港股搜索优先使用雪球（`stockn.xueqiu.com`），无结果时回退到同花顺（`notice.10jqka.com.cn`）
- 新三板（NEEQ）下载无需 `--url`，脚本自动搜索 `neeq.com.cn`（股转系统）；需携带正确的浏览器请求头以避免 403
- 下载脚本会验证 PDF 文件头（`%PDF-` magic bytes）
- 文件小于 100KB 时会发出警告（可能为异常文件）
- 下载超时 120 秒，最多重试 3 次（渐进式退避）
- 下载文件命名格式：`{股票代码}_{股票名称}_{报告类型}_{年份}.pdf`（如 `833442_江苏铁科_年报_2018.pdf`）。股票名称通过 `--stock-name` 传入；新三板（NEEQ）会自动从股转系统接口识别，无需手动指定
